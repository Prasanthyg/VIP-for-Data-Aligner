//create the coergroup intended to collect the info from the monitor -> create the analysis port via which we actually collect the info
//from the monitor, i.e connection to the monitor->instantiate it in the constructor and call the write function associaed with the port  
...
`ifndef CFS_APB_COVERAGE_SV
  ...
  //allows us to go to the convert2tring function in the coverage group and there we can attain access to all the children of
  //the coverage class which are ingerited from the base class
  ...
  class cfs_apb_coverage extends uvm_component implements cfs_apb_reset_handler;
    ...
    function new(string name = "", uvm_component parent);
      ...
      //instance of the covergroup in the constructor of the civerage componenet!
      cover_item = new();
	  cover_item.set_inst_name($sformatf("%s_%s", get_full_name(), "cover_item"));//this instance name is used to map the
      //cover item in the verification plan
    ...
    //Port associated with port_item port and the write function associated with this port. 
    //This is the function which is called by the monitor to send the collected information to the coverage component.
    virtual function void write_item(cfs_apb_item_mon item);//we have the _item in the function name because of the uvm_analysis_imp_item declaration macro, 
    //we can change the name of the function but not the suffix _item
      cover_item.sample(item);
      //in the agent class the analysis port port_item is connected to the monitor in the connect phase
    ...
    //   get_children(children);// this will give us access to all the components instantiated under 
    //   the coverage group and this is the case here for the wrapper classes
    ...
    //   return result;
    //again the motivation to resort to such foreach loop is-
    // endfunction
