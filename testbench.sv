...
module testbench();
    ...
    //instance of the md _rx and md_tx interfaces
    cfs_md_if #(ALGN_DATA_WIDTH) md_rx_if(.clk(clk));
    cfs_md_if #(ALGN_DATA_WIDTH) md_tx_if(.clk(clk));

    //assign the reset of these interfaces to the reset of the apb interface
    assign md_rx_if.reset_n = apb_if.preset_n;
    assign md_tx_if.reset_n = apb_if.preset_n;

    //connecting the rx/tx interfaces to the DUT Signals
    initial begin
        $dumpfile("dump.vcd");
        $dumpvars;
